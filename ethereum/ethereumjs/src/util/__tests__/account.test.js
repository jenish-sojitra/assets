...
import { privateToPublic } from '../extra.js'
import {
...
const eip1014Testdata = loadFixture('eip1014Examples')

/* eslint-disable no-new */

describe('Account', function () {
  tape('empty constructor', function (st) {
  ...
  tape('from Array data', function (st) {
    const raw = [
      '0x02', // nonce
      '0x0384', // balance
  ...
  tape('from Object data', function (st) {
    const raw = {
      nonce: '0x02',
      balance: '0x0384',
  ...
  tape('from RLP data', function (st) {
  ...
  tape('validation', function (st) {
    ...
    st.throws(() => {
      Account.fromRlpSerializedAccount(data)
...
describe('Utility Functions', function () {
  tape('isValidPrivate', function (st) {
  ...
  tape('isValidPublic', function (st) {
  ...
  tape('publicToAddress', function (st) {
    let pubKey = Buffer.from(
      '3a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a001cf066d56a8156fc201cd5df8a36ef694eecd258903fca7086c1fae7441e1d',
      'hex'
    )
  ...
  tape('privateToPublic', function (st) {
    const pubKey =
      '3a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a001cf066d56a8156fc201cd5df8a36ef694eecd258903fca7086c1fae7441e1d'
    ...
    try {
      ...
      st.notOk(err.message.includes(privateKey), 'not reflect private key in error')
    }

    st.end()
  })

  tape('privateToAddress', function (st) {
  ...
  tape('generateAddress', function (st) {
  ...
  tape('generateAddress wt.testh hex prefix', function (st) {
    const addr = generateAddress(
  ...
  tape('generateAddress wt.testh nonce 0 (special case)', function (st) {
    const addr = generateAddress(
      toBuffer('0x990ccf8a0de58091c028d6ff76bb235ee67c1c39'),
      toBuffer(0)
    )
  ...
  tape('generateAddress wt.testh non-buffer inputs', function (st) {
    st.throws(function () {
      generateAddress('0x990ccf8a0de58091c028d6ff76bb235ee67c1c39', toBuffer(0))
    }, 'should throw if address is not Buffer')

    st.throws(function () {
      generateAddress(toBuffer('0x990ccf8a0de58091c028d6ff76bb235ee67c1c39'), 0)
    }, 'should throw if nonce is not Buffer')
    st.end()
  })

  tape('generateAddress2: EIP-1014 testdata examples', function (st) {
  ...
  tape('generateAddress2: non-buffer inputs', function (st) {
    const { address, salt, initCode } = eip1014Testdata[0]

    st.throws(function () {
      generateAddress2(address, toBuffer(salt), toBuffer(initCode))
    }, 'should throw if address is not Buffer')

    st.throws(function () {
      generateAddress2(toBuffer(address), salt, toBuffer(initCode))
    }, 'should throw if salt is not Buffer')

    st.throws(function () {
      generateAddress2(toBuffer(address), toBuffer(salt), initCode)
  ...
  describe('toChecksumAddress()', function () {
    tape('EIP55', function (st) {
      for (const tmp of eip55ChecksumAddresses) {
    ...
    describe('EIP1191', function () {
      tape('Should encode the example addresses correctly', function (st) {
      ...
      tape('Should encode large chain ids greater than MAX_INTEGER correctly', function (st) {
  ...
  describe('isValidChecksumAddress()', function () {
    tape('EIP55', function (st) {
      for (const eip55ChecksumAddress of eip55ChecksumAddresses) {
        st.ok(isValidChecksumAddress(eip55ChecksumAddress))
      }
    ...
    describe('EIP1191', function () {
      tape('Should return true for the example addresses', function (st) {
        for (const [chainId, addresses] of Object.entries(eip1191ChecksummAddresses)) {
          for (const addr of addresses) {
      ...
      tape('Should return false for invalid cases', function (st) {
        // If we set the chain id, an EIP55 encoded address should be invalid
        for (const eip55ChecksumAddress of eip55ChecksumAddresses) {
          st.notOk(isValidChecksumAddress(eip55ChecksumAddress, 1))
        }
      ...
      tape('Should return false if the wrong chain id is used', function (st) {
        for (const [chainId, addresses] of Object.entries(eip1191ChecksummAddresses)) {
          for (const addr of addresses) {
      ...
      tape('Should return false if input is not hex-prefixed', function (st) {
  ...
  describe('isValidAddress()', function () {
    tape('should return true', function (st) {
    ...
    tape('should return false', function (st) {
