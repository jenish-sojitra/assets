...
describe('[Common]: Parameter access for param(), paramByHardfork()', function () {
  const t = { test: tape }
  ...
  t.test('Error cases for param(), paramByHardfork()', function (st) {
  ...
  t.test('Access by block number, paramByBlock()', function (st) {
    ...
    st.equal(c.paramByBlock('pow', 'minerReward', 4_370_000), '3000000000000000000', msg)

    msg = 'Should correctly translate block numbers into HF states (original value)'
    st.equal(c.paramByBlock('pow', 'minerReward', 4_369_999), '5000000000000000000', msg)

    st.comment('-----------------------------------------------------------------')
    st.end()
  })

  t.test('EIP param access, paramByEIP()', function (st) {
    const c = new Common({ chain: Chain.Mainnet })

    let msg = 'Should return null for non-existing value'
    st.equal(c.paramByEIP('gasPrices', 'notexistingvalue', 2537), null, msg)

    const UNSUPPORTED_EIP = 1_000_000
    let f = function () {
  ...
  t.test('returns the right block delay for EIP3554', function (st) {
    for (const fork of [Hardfork.MuirGlacier, Hardfork.Berlin]) {
      ...
      st.equal(delay, 9_000_000)
      c.setEIPs([3554])
      delay = c.param('pow', 'difficultyBombDelay')
      st.equal(delay, 9_500_000)
    }
