...
import { Chain, Common, Hardfork } from '../common.js'

describe('[Common]: Hardfork logic', function () {
  const t = { test: tape }
  ...
  t.test('getHardforkByBlockNumber() / setHardforkByBlockNumber()', function (st) {
    let c = new Common({ chain: Chain.Mainnet })
    let msg = 'should get HF correctly'

    st.equal(c.getHardforkByBlockNumber(0), 'chainstart', msg)
    st.equal(c.getHardforkByBlockNumber(1_149_999), 'chainstart', msg)
    st.equal(c.getHardforkByBlockNumber(1_150_000), 'homestead', msg)
    st.equal(c.getHardforkByBlockNumber(1_400_000), 'homestead', msg)
    st.equal(c.getHardforkByBlockNumber(9_200_000), 'muirGlacier', msg)
    st.equal(c.getHardforkByBlockNumber(12_244_000), 'berlin', msg)
    st.equal(c.getHardforkByBlockNumber(12_965_000), 'london', msg)
    st.equal(c.getHardforkByBlockNumber(999_999_999_999), 'london', msg)

    msg = 'should set HF correctly'

    st.equal(c.setHardforkByBlockNumber(0), 'chainstart', msg)
    st.equal(c.setHardforkByBlockNumber(1_149_999), 'chainstart', msg)
    st.equal(c.setHardforkByBlockNumber(1_150_000), 'homestead', msg)
    st.equal(c.setHardforkByBlockNumber(1_400_000), 'homestead', msg)
    st.equal(c.setHardforkByBlockNumber(12_244_000), 'berlin', msg)
    st.equal(c.setHardforkByBlockNumber(12_965_000), 'london', msg)
    st.equal(c.setHardforkByBlockNumber(999_999_999_999), 'london', msg)
  ...
  t.test('hardforkBlock()', function (st) {
    ...
    st.equal(c.hardforkBlock('byzantium'), 1_700_000, msg)
    ...
    st.equal(c.hardforkBlock(), 1_700_000, msg)
    ...
    st.equal(c.hardforkBlock(), 6_485_846, msg)
  ...
  t.test('isHardforkBlock()', function (st) {
    ...
    st.equal(c.isHardforkBlock(1_700_000, 'byzantium'), true, msg)

    msg = 'should return false for another block for byzantium (provided)'
    st.equal(c.isHardforkBlock(1_700_001, 'byzantium'), false, msg)

    c = new Common({ chain: Chain.Ropsten, hardfork: Hardfork.Byzantium })
    msg = 'should return true for HF change block for byzantium (set)'
    st.equal(c.isHardforkBlock(1_700_000), true, msg)

    msg = 'should return false for another block for byzantium (set)'
    st.equal(c.isHardforkBlock(1_700_001), false, msg)

    st.end()
  })

  t.test('nextHardforkBlock()', function (st) {
    ...
    st.equal(c.nextHardforkBlock('byzantium'), 3_660_663, msg)
  ...
  t.test('isNextHardforkBlock()', function (st) {
    ...
    st.equal(c.isNextHardforkBlock(3_660_663, 'byzantium'), true, msg)
    ...
    st.equal(c.isNextHardforkBlock(605_948_938, 'byzantium'), false, msg)
  ...
  t.test('hardforkIsActiveOnBlock() / activeOnBlock()', function (st) {
    ...
    st.equal(c.hardforkIsActiveOnBlock('byzantium', 1_700_000), true, msg)

    msg = 'Ropsten, byzantium (provided), 1700005 -> true'
    st.equal(c.hardforkIsActiveOnBlock('byzantium', 1_700_005), true, msg)

    msg = 'Ropsten, byzantium (provided), 1699999 -> false'
    st.equal(c.hardforkIsActiveOnBlock('byzantium', 1_699_999), false, msg)

    c = new Common({ chain: Chain.Ropsten, hardfork: Hardfork.Byzantium })
    msg = 'Ropsten, byzantium (set), 1700000 -> true'
    st.equal(c.hardforkIsActiveOnBlock(null, 1_700_000), true, msg)

    msg = 'Ropsten, byzantium (set), 1700000 -> true (alias function)'
    st.equal(c.activeOnBlock(1_700_000), true, msg)

    msg = 'Ropsten, byzantium (set), 1700005 -> true'
    st.equal(c.hardforkIsActiveOnBlock(null, 1_700_005), true, msg)

    msg = 'Ropsten, byzantium (set), 1699999 -> false'
    st.equal(c.hardforkIsActiveOnBlock(null, 1_699_999), false, msg)

    st.end()
  })

  t.test('hardforkBlock() / hardforkBlockBN()', function (st) {
    const c = new Common({ chain: Chain.Mainnet })

    let msg = 'should return correct value'
    st.equal(c.hardforkBlock(Hardfork.Berlin), 12_244_000, msg)
    st.ok(c.hardforkBlockBN(Hardfork.Berlin).eq(new BN(12_244_000)), msg)
  ...
  t.test('hardforkGteHardfork() ropsten', function (st) {
  ...
  t.test('_calcForkHash()', function (st) {
    let c = new Common({ chain: Chain.Mainnet })
  ...
  t.test('hardforkForForkHash()', function (st) {
    const c = new Common({ chain: Chain.Mainnet })
