...
import { Chain, Common, CustomChain, Hardfork } from '../common.js'
import loadFixture from './load-fixture.cjs'

const testnet = loadFixture('testnet')
const testnet2 = loadFixture('testnet2')
const testnet3 = loadFixture('testnet3')

describe('[Common]: Custom chains', function () {
  const t = { test: tape }
  ...
  t.test(
    ...
    function (st) {
      ...
      st.equal(c.chainId(), 12_345, 'should return correct chain Id')
      st.ok(c.chainIdBN().eqn(12_345), 'should return correct chain Id')
      st.equal(c.networkId(), 12_345, 'should return correct network Id')
      st.ok(c.networkIdBN().eqn(12_345), 'should return correct network Id')
      st.equal(
        c.genesis().hash,
  ...
  t.test('custom() -> base functionality', function (st) {
  ...
  t.test('custom() -> behavior', function (st) {
  ...
  t.test('forCustomChain() (deprecated) -> base functionality', function (st) {
    const mainnetCommon = new Common({ chain: Chain.Mainnet })
  ...
  t.test('customChains parameter: initialization', (st) => {
    ...
    st.equal(c.hardforkBlock(), 4_370_000, 'customChains, chain set to supported chain')
