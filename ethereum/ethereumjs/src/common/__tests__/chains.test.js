...
describe('[Common/Chains]: Initialization / Chain params', function () {
  const t = { test: tape }
  ...
  t.test('Should initialize with chain provided by Chain enum', function (st) {
  ...
  t.test('Should provide correct access to chain parameters', function (st) {
    ...
    st.equal(c.hardforks()[3]['block'], 2_463_000, 'should return correct hardfork data')
    /*
    ...
    */
    ...
    st.equal(c.consensusConfig().epoch, 30_000, 'should return correct consensus config parameters')
    st.end()
  })

  /*
  t.test('Should provide the bootnode information in a uniform way', function (st) {
  ...
  t.test('Should provide DNS network information in a uniform way', function (st) {
    const configs = ['mainnet', 'ropsten', 'rinkeby', 'goerli']
    for (const network of configs) {
      const c = new Common({ chain: network })
...
describe('[Common]: isSupportedChainId static method', function () {
  const t = { test: tape }

  t.test('Should return true for supported chainId', function (st) {
    st.equal(Common.isSupportedChainId(new BN(1)), true, 'returns true')
    st.end()
  })

  t.test('Should return false for unsupported chainId', function (st) {
