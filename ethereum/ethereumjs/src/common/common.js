...
import { toType, TypeOutput } from '../util/extra.js'
import { intToBuffer } from '../util/index.js'
import { _getInitializedChains } from './chains/index.cjs'
import { EIPs } from './eips/index.cjs'
import { hardforks as HARDFORK_CHANGES } from './hardforks/index.cjs'

const { buf: crc32Buffer } = crc32
...
export class Common extends EventEmitter {
  ...
  static custom(chainParamsOrName, opts = {}) {
    ...
    const standardChainParams = Object.assign(
      Object.create(null),
  ...
  static isSupportedChainId(chainId) {
    const initializedChains = _getInitializedChains()
  ...
  setEIPs(eips = []) {
    for (const eip of eips) {
      ...
      if (EIPs[eip].requiredEIPs) {
        ...
        (EIPs[eip].requiredEIPs).forEach((elem) => {
    ...
    for (const hfChanges of HARDFORK_CHANGES) {
      ...
      if (hfChanges[1].hasOwnProperty('eips')) {
        ...
        for (const eip of hfEIPs) {
          ...
          value = valueEIP === null ? value : valueEIP
        }
   ...
   * manually passed in with the eips or in a
  ...
  isActivatedEIP(eip) {
    ...
    for (const hfChanges of HARDFORK_CHANGES) {
      ...
      if (this.gteHardfork(hf['name']) && 'eips' in hf && hf['eips'].includes(eip)) {
        return true
      }
    }
  ...
  hardforkIsActiveOnBlock(hardfork, blockNumber, opts = {}) {
    ...
    return Boolean(hfBlock && blockNumber.gte(hfBlock))
  }
   ...
   * @param opts Hardfork options (onlyActive unused)
   * @returns Array with hardfork arrays
   */
  ...
  nextHardforkBlockBN(hardfork) {
    ...
    return this.hardforks().reduce((acc, hf) => {
  ...
  _calcForkHash(hardfork) {
    const genesis = Buffer.from(this.genesis().hash.slice(2), 'hex')
  ...
  hardforkForForkHash(forkHash) {
    ...
    return resArray.length > 0 ? resArray[resArray.length - 1] : null
  }
