...
const { normalizeSuiAddress, normalizeSuiObjectId, SUI_TYPE_ARG } = vendorLib.utils

const { chunk } = vendorLib.utils

const MAX_OBJECTS_PER_FETCH = 50
const GAS_SAFE_OVERHEAD = BigInt(1000)
const MAX_GAS = 5e10
...
async function setGasBudget(transactionData, client) {
  ...
  const safeOverhead = GAS_SAFE_OVERHEAD * BigInt(transactionData.gasConfig.price || BigInt(1))
...
async function resolveObjectReferences(transactionData, client) {
  ...
  const objectChunks = dedupedIds.length > 0 ? chunk(dedupedIds, MAX_OBJECTS_PER_FETCH) : []
  const resolved = (
    await Promise.all(
      objectChunks.map((chunk2) =>
  ...
  const invalidObjects = [...responsesById]
...
async function normalizeInputs(transactionData, client) {
  ...
  if (moveCallsToResolve.length > 0) {
    await Promise.all(
      moveCallsToResolve.map(async (moveCall) => {
        ...
        const params = hasTxContext ? parameters.slice(0, -1) : parameters
...
export { suiClientResolveTransactionPlugin }
