...
import { is, parse } from '../valibot.js'
import { SuiCommands } from './Commands.js'
...
const { fromBase64, normalizeSuiAddress } = vendorLib.utils
const { isSerializedBcs } = vendorLib.bcs
...
const _Transaction = class _Transaction {
  constructor() {
    ...
    this.object = createObjectMethods((value) => {
      ...
      return inserted
        ...
        : __privateMethod(this, _Transaction_instances, addInput_fn).call(
            this,
            'object',
            typeof value === 'string'
              ? {
  ...
  static from(transaction) {
    ...
    if (isTransaction(transaction)) {
      ...
      __privateSet(
        newTransaction,
        _data,
        TransactionDataBuilder.fromBytes(
    ...
    __privateSet(
      newTransaction,
      _availableResults,
      new Set(__privateGet(newTransaction, _commandSection).map((_, i) => i))
    )
  ...
  addIntentResolver(intent, resolver) {
    if (
      __privateGet(this, _intentResolvers).has(intent) &&
      __privateGet(this, _intentResolvers).get(intent) !== resolver
    ) {
  ...
  setExpiration(expiration) {
    __privateGet(this, _data).expiration = expiration
  ...
  setGasPayment(payments) {
    __privateGet(this, _data).gasConfig.payment = payments.map((payment) =>
  ...
  get pure() {
    Object.defineProperty(this, 'pure', {
      ...
      value: createPure((value) => {
        ...
        return __privateMethod(this, _Transaction_instances, addInput_fn).call(
          this,
          'pure',
  ...
  splitCoins(coin, amounts) {
    const command = SuiCommands.SplitCoins(
      typeof coin === 'string'
        ? this.object(coin)
        : __privateMethod(this, _Transaction_instances, resolveArgument_fn).call(this, coin),
      amounts.map((amount) =>
        typeof amount === 'number' || typeof amount === 'bigint' || typeof amount === 'string'
          ? this.pure.u64(amount)
          : __privateMethod(this, _Transaction_instances, normalizeTransactionArgument_fn).call(
              this,
              amount
            )
      )
  ...
  async toJSON(options = Object.create(null)) {
  ...
  isFullyResolved() {
    ...
    if (needsTransactionResolution(__privateGet(this, _data), Object.create(null))) {
      return false
    }
  ...
  async build(options = Object.create(null)) {
    await this.prepareForSerialization(options)
  ...
  async getDigest(options = Object.create(null)) {
    await this.prepareForSerialization(options)
    await __privateMethod(this, _Transaction_instances, prepareBuild_fn).call(this, options)
...
addCommand_fn = function (command) {
  ...
  __privateGet(this, _data).mapCommandArguments(resultIndex, (arg) => {
    ...
    if (
      ...
      !__privateGet(this, _availableResults).has(arg.NestedResult[0])
    ) {
      throw new Error(
...
prepareBuild_fn = async function (options) {
  ...
  await __privateMethod(this, _Transaction_instances, runPlugins_fn).call(
    this,
    [...__privateGet(this, _buildPlugins), resolveTransactionPlugin],
    options
  )
}
