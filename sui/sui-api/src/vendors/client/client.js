...
const { fromBase58, toBase64, toHex } = vendorLib.utils
...
const { isValidNamedPackage } = vendorLib.utils
...
class SuiClient extends Experimental_BaseClient {
  ...
  async getRpcApiVersion({ signal } = Object.create(null)) {
   ...
   */
  async call(method, params, { signal } = Object.create(null)) {
   ...
   */
  async getTotalTransactionBlocks({ signal } = Object.create(null)) {
    const resp = await this.transport.request({
   ...
   */
  async getReferenceGasPrice({ signal } = Object.create(null)) {
    const resp = await this.transport.request({
   ...
   */
  async getLatestSuiSystemState({ signal } = Object.create(null)) {
    return await this.transport.request({
  ...
  async devInspectTransactionBlock(input) {
    ...
    if (isTransaction(input.transactionBlock)) {
      ...
      throw new TypeError('Unknown transaction block format.')
    }
   ...
   */
  async getLatestCheckpointSequenceNumber({ signal } = Object.create(null)) {
    const resp = await this.transport.request({
  ...
  async getNetworkMetrics({ signal } = Object.create(null)) {
    return await this.transport.request({
  ...
  async getAddressMetrics({ signal } = Object.create(null)) {
    return await this.transport.request({
   ...
   */
  async getMoveCallMetrics({ signal } = Object.create(null)) {
    return await this.transport.request({
   ...
   * Return the committee information for the asked epoch
   */
  async getCurrentEpoch({ signal } = Object.create(null)) {
    return await this.transport.request({
   ...
   */
  async getValidatorsApy({ signal } = Object.create(null)) {
    return await this.transport.request({
  ...
  async getChainIdentifier({ signal } = Object.create(null)) {
